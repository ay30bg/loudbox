...
const handlePayment = async () => {
  ...
  const handler = window.PaystackPop.setup({
    key: process.env.REACT_APP_PAYSTACK_PUBLIC_KEY || 'pk_test_6b703a5955176568e45c9fd67a879c0a6d7f9842',
      ...
      if (response.status === 'success') {
        ...
        try {
          ...
          if (!emailResponse.ok) {
            ...
            alert('Payment successful, but failed to send ticket email. Please contact support.');
          } else {
            console.log('Ticket email sent:', emailResult.message);
          }
        } catch (error) {
