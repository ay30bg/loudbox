...
const OrderSummary = ({ eventData, totalPrice, ticketQuantity, navigateBack }) => {
  const [isPaying, setIsPaying] = useState(false);
  const [paymentError, setPaymentError] = useState(null);
  const [email, setEmail] = useState('loudbox.tickets@gmail.com');
  const [firstName, setFirstName] = useState('Eniola');
  const [lastName, setLastName] = useState('Yekeen');
  const [phoneNumber, setPhoneNumber] = useState('+2349034951446');
  ...
  const paystackProps = {
    email: email || 'guest@example.com',
    amount: totalPrice * 100,
    publicKey: process.env.REACT_APP_PAYSTACK_PUBLIC_KEY || 'pk_test_f5af6c1a30d2bcfed0192f0e8006566fe27441df',
    text: isPaying ? 'Processing...' : `Pay NGN ${totalPrice.toLocaleString()}`,
